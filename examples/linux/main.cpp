...
// #define UMBRA_ENABLE_LOGGING
#define UMBRA_RUNTIME_MEMORY_BOUNDS_CHECK
...
int main(int argc, char** argv) {
  ...
  umbra::machine_t machine{128 * 1024 * 1024, offset};
  ...
  while (1) {
    ...
    if (timercmp) {
      if (timer > timercmp) {
        machine._wfi = false;
        machine.handle_trap(umbra::exception_code_t::e_machine_timer_interrupt,
                            0);
      }
    }
  }
