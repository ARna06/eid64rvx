...
umbra::machine_t* load_elf(const std::filesystem::path& path) {
  ELFIO::elfio reader;
  ...
  umbra::machine_t* machine = new umbra::machine_t{16 * 1024 * 1024, guest_base};
...
int main(int argc, char** argv) {
  ...
  umbra::machine_t* machine = load_elf(argv[1]);
  if (!machine) return -1;  // TODO: throw

  machine->_syscalls[93] = [](umbra::machine_t& machine) {
